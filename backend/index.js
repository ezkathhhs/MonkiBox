...
const { pool, testConnection } = require('./db'); // Importamos el 'pool'
...
app.get('/api/reports/category-distribution', async (req, res) => {
  try {
...
app.get('/api/reports/product/:id', async (req, res) => {
  try {
    const { id } = req.params;
